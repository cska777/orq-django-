        ...
        if response.status_code == 200:
            ...
            for movie_div in soup.select('li.mdl'):
                try:
                    ...
                    if meta_info_elem:
                        ...
                        if len(info_parts) >= 2:
                            ...
                            if date_sortie_elem:
                                ...
                                date_sortie = datetime.strptime(date_sortie, '%d %B %Y').strftime('%d %B %Y')
                            ...
                            movie_info['duree_originale'] = duree
                    ...
                    # Genres du film
                    genres_elem = movie_div.select_one('.meta-body-item.meta-body-info')
                    if genres_elem:
                        genres_text = genres_elem.text.strip()
                        genres_parts = genres_text.split('|')
                        if len(genres_parts) >= 3:
                            genres = [genre.strip() for genre in genres_parts[2].split(',')]
